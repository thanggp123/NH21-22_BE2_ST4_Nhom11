...
<head>
	...
	<!-- Slick -->
	<link type="text/css" rel="stylesheet" href="{{ asset('css/banner.css') }}">
		...
		<div id="top-header">
			<div class="container">
				<ul class="header-links pull-left">
					...
					<li><a href="#"><i class="fa fa-map-marker"></i> 1933 Stonecoal Road</a></li>
				</ul>
				<ul class="header-links pull-right">
					<?php if ($user != NULL) {
						if ($user->name == "admin") { ?>
							<li><a href="{{ url('/admin') }}"><i class="fa fa-user-o"></i>Xin Chào {{ $user->name }}</a></li>
						<?php } else { ?>
							<li><a href="{{ url('/') }}"><i class="fa fa-user-o"></i>Xin Chào {{ $user->name }}</a></li>
						<?php } ?>
						<li>
							<form method="POST" action="{{ route('logout') }}">
								@csrf
								<a class="nav-link" href="{{ route('logout') }}" onclick="event.preventDefault(); this.closest('form').submit();" style="text-align: center; color: aliceblue">{{ __('Đăng Xuất') }} <i class="fa fa-arrow-circle-right"></i></a>
							</form>
						</li>
					<?php } else { ?>
						<li><a href="{{ url('/login') }}"><i class="fa fa-user-o"></i>Đăng Nhập</a></li>
						<li><a href="{{ url('/register') }}"><i class="fa fa-user-o"></i>Đăng Ký</a></li>
					<?php } ?>
				</ul>
			</div>
		</div>
		...
		<div class="container">
			...
			<div id="responsive-nav">
				...
				<ul class="main-nav nav navbar-nav">
					...
					<li><a href="{{ url('/search?option=alls') }}">Tất Cả Sản Phẩm</a></li>
